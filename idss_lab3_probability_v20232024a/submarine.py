...
# 潜艇的概率分布函数 (PMF)
def make_pmf():
    blobs = [(5, 5, 2), (10, 3, 3), (8, 5, 2), (9, 7, 1), (6, 6, 1.0)] # 潜艇的位置和强度

    _pmf = np.zeros((16, 16))

    for i in range(16):# 计算每个网格点的概率
        for j in range(16):
            for x, y, r in blobs:
                ix = i - x
                jy = j - y
    ...
    _pmf = _pmf / np.sum(_pmf)# 标准化使得总和为 1
    return _pmf
...
def make_other_pmfs():
    ...
    return proposed_pmfs # 将不同类型的 PMF 存入列表
...
def sample_submarine(n, pmf=submarine_pmf):#潜艇位置样本
